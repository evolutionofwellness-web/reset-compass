// app.js v103 — robust startup, full event binding, realistic arrow, scroll-driven spin retained
function escapeHtml(s){ return String(s||'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch])); }
function escapeJs(s){ return String(s||'').replace(/'/g,"\\'").replace(/"/g,'\\"'); }

// small diagnostic capture
...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    // ROUTES available as function declarations below — renderRoute will call them
    window.addEventListener('hashchange', renderRoute);

    // Scroll-driven rotation setup (0 -> 720deg)
    ...
    // Wedge interactions and keyboard support
    if (compass) {
      document.querySelectorAll('#compass path[data-mode]').forEach(w => {
        const angle = Number(w.getAttribute('data-angle') || 0);
        w.setAttribute('tabindex','0');
        w.addEventListener('pointerenter', () => { setInteractionPause(true); needleGroup && needleGroup.classList.remove('idle'); needleGroup && (needleGroup.style.transform = `rotate(${angle}deg)`); w.classList.add('bloom'); });
        w.addEventListener('pointerleave', () => { w.classList.remove('bloom'); setInteractionPause(false); clearTimeout(resumeTimer); resumeTimer = setTimeout(()=>{ if(!scrollActive && needleGroup) needleGroup.classList.add('idle'); }, 260); });
        w.addEventListener('pointerdown', (e) => { e.preventDefault(); setInteractionPause(true); needleGroup && needleGroup.classList.remove('idle'); needleGroup && (needleGroup.style.transform = `rotate(${angle}deg)`); w.classList.add('bloom'); }, {passive:false});
        w.addEventListener('click', (e) => {
          e.preventDefault();
          w.classList.remove('bloom'); w.classList.add('bloom-strong');
          if (needleGroup) {
            needleGroup.classList.remove('idle'); needleGroup.classList.add('needle-spin');
            setTimeout(()=>{ needleGroup.classList.remove('needle-spin'); needleGroup.classList.add('idle'); w.classList.remove('bloom-strong'); const mode = w.getAttribute('data-mode'); if (mode) navigateMode(mode); setInteractionPause(false); }, 560);
          } else {
            const mode = w.getAttribute('data-mode'); if (mode) navigateMode(mode); setInteractionPause(false);
          }
        });
        w.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); const m = w.getAttribute('data-mode'); if (m) navigateMode(m); } });
      });
    }

    // Touch pointer heuristics (tap-to-select safety)
    if (compass) {
    ...
    // Ensure buttons respond (defensive)
    document.querySelectorAll('button[data-mode]').forEach(b => b.addEventListener('click', (e) => {
      const m = b.getAttribute('data-mode'); if (m) navigateMode(m);
    }));
    ...
    // small console notice
    console.info('App initialized (v103). If interactions fail, open console and check window.__lastAppError');
  } catch (err) {
    console.error('App startup error:', err);
    window.__lastAppError = { msg: err.message || String(err), stack: err.stack || null, time: new Date().toISOString() };
    // ensure UI visible so user is not stuck
...
/* --- Activities, renders and behavior functions (unchanged baseline implementations) --- */
