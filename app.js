// app.js v107 — no splash animation blocking, robust bindings, stylized arrow retained
...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    // Ensure root visible (no splash)
    if (appRoot) { appRoot.classList.add('visible'); appRoot.setAttribute('aria-hidden','false'); }

    // Ensure needle is non-interactive and idle marker set
    ...
    document.querySelectorAll('.nav-links a[data-hash]').forEach(a => {
      a.addEventListener('click', (e) => { e.preventDefault(); const h = a.getAttribute('data-hash') || a.getAttribute('href'); console.debug('nav ->', h); if (h) navigateHash(h); });
    });

    // Mode buttons
    document.querySelectorAll('button[data-mode]').forEach(b => {
      b.addEventListener('click', (e) => { const m = b.getAttribute('data-mode'); console.debug('mode button ->', m); if (m) navigateMode(m); });
    });
    ...
    // Wedges: hover, pointerdown, click (defensive)
    if (compass) {
      const wedges = Array.from(document.querySelectorAll('#compass path[data-mode]'));
      console.debug('wedges bound:', wedges.length);
      wedges.forEach(w => {
        const angle = Number(w.getAttribute('data-angle') || 0);
        w.setAttribute('tabindex','0');
        w.addEventListener('pointerenter', ()=>{ setInteractionPause(true); if (needleGroup) needleGroup.classList.remove('idle'); needleGroup && (needleGroup.style.transform = `rotate(${angle}deg)`); w.classList.add('bloom'); });
        ...
        w.addEventListener('pointerdown', (e)=>{ try{ e.preventDefault(); }catch(e){} setInteractionPause(true); if (needleGroup) needleGroup.classList.remove('idle'); needleGroup && (needleGroup.style.transform = `rotate(${angle}deg)`); w.classList.add('bloom'); }, { passive:false });
        w.addEventListener('click', (e)=>{ try{ e.preventDefault(); }catch(e){} console.debug('wedge click ->', w.getAttribute('data-mode')); w.classList.remove('bloom'); w.classList.add('bloom-strong'); if (needleGroup){ needleGroup.classList.remove('idle'); needleGroup.classList.add('needle-spin'); setTimeout(()=>{ needleGroup.classList.remove('needle-spin'); needleGroup.classList.add('idle'); w.classList.remove('bloom-strong'); const mode = w.getAttribute('data-mode'); if (mode) navigateMode(mode); setInteractionPause(false); },560); } else { const mode = w.getAttribute('data-mode'); if (mode) navigateMode(mode); setInteractionPause(false); } });
    ...
    renderRoute();
    updateStreak();
    console.info('App v107 initialized — check Console for binding logs.');
  } catch (err) {
...
/* --- render/behavior functions (stable baseline) --- */
