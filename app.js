...
(function () {
  ...
  function ensureContentElement(){
    var c = id('content'); if(c) return c;
    var page = id('page') || document.querySelector('.page') || document.body;
    c = document.createElement('div'); c.id='content'; c.className='content-area'; c.setAttribute('aria-live','polite'); page.appendChild(c); return c;
  }
  ...
  function runConfetti(mode){
    try{
      ...
      for(var i=0;i<n;i++){ var dot=document.createElement('div'); var size=(6+Math.round(Math.random()*8))+'px'; dot.style.width=size; dot.style.height=size; dot.style.borderRadius='50%'; dot.style.background=colors[i%colors.length]; dot.style.margin=(Math.random()*12)+'px'; container.appendChild(dot); }
  ...
  window.completeActivity = function(mode, activity, noteId, rowId){
    try{
      var row = id(rowId); if(row){ try{ row.classList.add('activity-complete-pop'); }catch(e){} }
      var noteElem = document.getElementById(noteId); var note = noteElem ? noteElem.value : '';
      ...
      var lastLogged = localStorage.getItem('lastLogged'); var today = new Date().toLocaleDateString();
      if(lastLogged !== today){ var streak = parseInt(localStorage.getItem('streak')||'0',10) || 0; streak += 1; localStorage.setItem('streak', String(streak)); localStorage.setItem('lastLogged', today); updateStreak(); }
  ...
  function renderHistory(){
    ...
    var history = JSON.parse(localStorage.getItem('resetHistory') || '[]'); var listHtml='';
    ...
    try{
      ...
      sessionState.timerId = setInterval(function(){
        ...
        if(sessionState.remaining <= 0){
          stopTimer(); sessionState.running = false; clearPersistedSession(sessionState.mode);
  ...
  function pushGuidedSessionHistory(mode){
    try{
      var date = new Date().toLocaleDateString(); var entry = { date: date, mode: mode, activity: 'Guided session complete', note: '' };
      var hist = JSON.parse(localStorage.getItem('resetHistory') || '[]'); hist.unshift(entry); localStorage.setItem('resetHistory', JSON.stringify(hist));
      var lastLogged = localStorage.getItem('lastLogged'); var today = new Date().toLocaleDateString();
      if(lastLogged !== today){ var streak = parseInt(localStorage.getItem('streak')||'0',10)||0; streak += 1; localStorage.setItem('streak', String(streak)); localStorage.setItem('lastLogged', today); updateStreak(); }
    }catch(e){}
  ...
  function createSessionBindings(mode){
    try{
      ...
      var start = id('session-start'), cancel = id('session-cancel'), closeBtn = id('session-close'), completeBtn = id('session-complete');
      ...
      if(completeBtn && modal._completeFn) completeBtn.removeEventListener('click', modal._completeFn);
      ...
      if(!modal.__checklistDelegate){
        modal.__checklistDelegate = function(e){
          try{
            var tgt = e.target; var row = tgt && tgt.closest ? tgt.closest('.step-item') : null; if(!row) return;
            var cb = row.querySelector('input[type="checkbox"]'); if(!cb) return; if(e.target === cb) return;
            e.preventDefault && e.preventDefault(); cb.checked = !cb.checked;
            try{ var modeTitle = id('session-title'); var modeName = modeTitle ? (modeTitle.textContent || '').split('—')[0].trim().toLowerCase() : 'session'; var list = Array.prototype.slice.call(modal.querySelectorAll('.step-item input[type="checkbox"]')).map(function(chk){ return chk.checked; }); localStorage.setItem('session.checklist.' + modeName, JSON.stringify(list)); }catch(err){}
          }catch(err){}
        };
  ...
  (function attachEarlyDelegate(){
    try{
      ...
      function safeNavigateMode(m){ try{ if(typeof window.navigateMode === 'function'){ window.navigateMode(m); return; } location.hash = '#mode/' + m; if(typeof window.renderRoute === 'function') window.renderRoute(); }catch(e){} }
      ...
      document.addEventListener('click', function(ev){
        try{
          ...
          if(svgNode){ var md = svgNode.getAttribute && svgNode.getAttribute('data-mode'); if(md){ try{ ev.preventDefault && ev.preventDefault(); }catch(e){} try{ setTheme(md); }catch(e){} safeNavigateMode(md); return; } }
  ...
  function attachCompassDelegation(){
    ...
    comp.addEventListener('click', function(ev){ try{ var tgt = ev.target; var node = (tgt && tgt.closest) ? tgt.closest('[data-mode]') : null; if(!node) return; var mode = node.getAttribute('data-mode'); setTheme(mode); renderModePage(mode); navigateMode(mode); }catch(e){ console.warn('compass click', e); } });
    comp.__delegated = true;
  }

  function setupNeedleSpin(){ var ng = id('needle-group'); if(!ng) return; var ANG=720, lastY=0, queued=false; function onScroll(){ lastY = window.scrollY || window.pageYOffset || document.documentElement.scrollTop || 0; if(!queued){ queued=true; window.requestAnimationFrame(function(){ queued=false; var doc=document.documentElement; var max=Math.max(1, doc.scrollHeight - window.innerHeight); var r = Math.max(0, Math.min(1, lastY/max)); try{ ng.style.transform = 'rotate(' + (r*ANG) + 'deg)'; }catch(e){} }); } } window.addEventListener('scroll', onScroll, { passive: true }); }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    try{
      ...
      ensureContentElement(); bindUI(); attachCompassDelegation(); setupNeedleSpin();
      if(!location.hash) location.hash = '#home';
      renderRoute(); updateStreak();
