/* app.js v122 — per-mode themes + quick-win theme + mode-specific confetti colors
   ...
   - renderRoute applies themes for mode pages and quick wins
*/
...
(function () {
  ...
  // expose current mode and confetti colors
  ...
  function runConfetti(mode) {
    try {
      ...
      for (var i = 0; i < n; i++) {
        ...
        (function(dot){ var angle = Math.random()*Math.PI*2; var dist = 60 + Math.random()*120; var dx = Math.cos(angle)*dist; var dy = Math.sin(angle)*dist; try { dot.animate([{ transform: 'translate(0,0) scale(1)', opacity: 1 }, { transform: 'translate(' + dx + 'px,' + dy + 'px) scale(0.9)', opacity: 0.9 }], { duration: 700 + Math.random()*300 }); } catch(e) {} })(dot);
      }
  ...
  window.completeActivity = function(mode, activity, noteId, rowId) {
    try {
      ...
      var date = new Date().toLocaleDateString(); var normalized = (mode === 'quick' || mode === 'quick-win') ? 'quick-win' : mode;
  ...
  function renderRoute(){
    try {
      var h = location.hash || '#home'; var isFull = h !== '#home';
  ...
  function renderHome(){ var c = ensureContentElement(); c.innerHTML = ''; }
  ...
  function renderModePage(mode){
    ...
    for (var i=0;i<activities[mode].length;i++){ var act = activities[mode][i];
  ...
  function renderHistory(){
    ...
    for(var j=0;j<history.length;j++){ var k = (history[j].mode||'').toLowerCase(); if (typeof counts[k] !== 'undefined') counts[k]++; }
  ...
  function attachCompassDelegation(){
    ...
    comp.addEventListener('click', function(ev){ try{ var tgt = ev.target; var path = (tgt && tgt.closest) ? tgt.closest('path[data-mode]') : null; if(!path) return; var mode = path.getAttribute('data-mode'); setTheme(mode); renderModePage(mode); navigateMode(mode); } catch(e){ console.warn('compass click', e); } });
  ...
  document.addEventListener('DOMContentLoaded', function(){
    try {
      ...
      renderRoute();
      updateStreak();
      console.info('[app v122] initialized (per-mode patterns + confetti colors)');
    } catch(err) {
